...
def run_recbole(
    ...
    for id, tid, cid, bid, p, r in zip(
        item_id, title_id, catergory_id, brand_id, price, rank
    ):
        id = int(id)
        tid = int(tid)
        cid = int(cid)
    ...
    for id, tid, cid, cit, sid, aid in zip(
        item_id, title_id, catergory_id, city_id, state_id, address_id
    ):
        id = int(id)
        tid = int(tid)
        cid = int(cid)
    ...
    writer.write("%s\t%s\t%s\n" % ("user_id", "seq", "target"))
    for user_id, seq_list, target in zip(
        train_dict["user_id"], train_dict["business_id_list"], train_dict["business_id"]
    ):
        uid = int(user_id)
        writer.write("%d\t" % uid)
        for id in seq_list:
    ...
    writer.write("%s\t%s\t%s\n" % ("user_id", "seq", "target"))
    for user_id, seq_list, target in zip(
        valid_dict["user_id"], valid_dict["business_id_list"], valid_dict["business_id"]
    ):
        uid = int(user_id)
        writer.write("%d\t" % uid)
        for id in seq_list:
    ...
    writer.write("%s\t%s\t%s\n" % ("user_id", "seq", "target"))
    for user_id, seq_list, target in zip(
        test_dict["user_id"], test_dict["business_id_list"], test_dict["business_id"]
    ):
        uid = int(user_id)
        writer.write("%d\t" % uid)
        for id in seq_list:
